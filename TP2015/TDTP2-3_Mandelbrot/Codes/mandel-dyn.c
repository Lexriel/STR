...
#define TAG_END  2
  ...
  if( p == r_master) {
     ...
     ***** d'autres requetes : il reste "nbloc-bloc" blocs a calculer */
    for ( ; bloc < nbloc; bloc++) {
      ...
      MPI_Send( &bloc, 1, MPI_INT, source, TAG_REQ, MPI_COMM_WORLD); 
    }
    
 
    /***** Reception des derniers blocs : on attend le meme nombre de
     ***** message que ceux envoyés dans la premiere boucle (i.e. np-1) */
    for ( dest = 0; dest < np-1 ; dest++ ) {
      /* Reception d'un numéro de bloc */
      MPI_Recv( &i, 1, MPI_INT, MPI_ANY_SOURCE, TAG_REQ, MPI_COMM_WORLD, &status);
      source = status.MPI_SOURCE;
      ...
      printf("Processus %d : envoi d'une terminaison à dest=%d\n", p, source); 
    ...
    for( ;;) {
      ...
      MPI_Recv(&bloc, 1, MPI_INT, r_master, MPI_ANY_TAG, MPI_COMM_WORLD, &status);	  
      printf("Processus %d : réception d'un message d'étiquette=%s\n", p, 
	     (status.MPI_TAG == TAG_REQ ? "TAG_REQ" : 
	      (status.MPI_TAG == TAG_DATA ? "TAG_DATA" : 
	       (status.MPI_TAG == TAG_END ? "TAG_END" : "inconnue!"))));
