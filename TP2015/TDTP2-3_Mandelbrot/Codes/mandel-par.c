...
#define R_MASTER     0   /* Rang du processeur maitre 
  ...
  /* Attention : si le MPI_Gather implique une sychronisation de tous les processus (dépend de l'implémentation 
   * et de la taille des messages), tous les processus vont terminer en meme temps : à éviter donc... */
  ...
  if( p == R_MASTER) {
    ...
    for( source = 1; source < np; source ++){
      ...
      printf("Reçu %d lignes commençant à la ligne %d du processus %d\n",
    ...
    /* Cette version permet de récupérer en premier les résultats des processus 
     * qui ont terminé avant les autres (sans tenir compte de leur rang). 
     * On a besoin de MPI_Probe() pour connaître l'émetteur. 
     * (puis MPI_Recv() pour récupérer le message correspondant). */
    for( source = 1; source < np; source ++){
      int s = 0;
